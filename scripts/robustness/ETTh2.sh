...
# =============================================================================
# Robustness runner for ETTh2 (Pamba-style per-dataset script)
# =============================================================================
# Trains via the project's default entry point (``python -m src.train``) with
# the tuned per-dataset config ``configs/ETTh2.yaml`` (canonical ETTh split
# included), then evaluates the test split under additive Gaussian noise
# (``--noise_level``): x_tilde = x + N(0, sigma^2) on the standardized input
# window only. Targets / window stats are never corrupted; training and
# validation always see clean inputs (see src/train.py:evaluate).
#
# Environment:
...
#
# Usage:
#   noise_level=0.2 bash scripts/robustness/ETTh2.sh [96|192|336|720|all]
#
